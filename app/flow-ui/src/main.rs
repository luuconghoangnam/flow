...
use download_row_vm::{current_downloads_menu_presentation_from_snapshot, current_home_action_registry_from_snapshot, current_home_action_state_from_snapshot};
use home_action_descriptors::{DownloadsMenuPresentation, HomeActionId};
...
use queue_refresh_pipeline::{DOWNLOAD_PROGRESS_CACHE, LAST_QUEUE_SNAPSHOT, enqueue_queue_refresh, estimate_speed_bytes_per_sec, queue_state_fallback, refresh_queue_ui, update_progress_cache};
...
fn open_file_checksum_dialog(payload: FileChecksumDialogPayload, weak: slint::Weak<MainWindow>) {
    if let Ok(dialog) = FileChecksumDialog::new() {
        ...
        for (index, item) in payload.items.iter().enumerate() {
            ...
            dialog.set_rows(ModelRc::new(VecModel::from(rows.clone())));
            dialog.set_status_text(format!("Checking {} ({}/{})", item.id, done + 1, payload.items.len()).into());
