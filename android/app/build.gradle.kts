...
import com.flowspeed.lib.installer.InstallerTargetFormat
import com.flowspeed.lib.plugin.common_android.task.SignApkTask
import com.flowspeed.lib.plugin.common_android.task.androidEnableFileTypesGeneratorForManifest
...
android {
    ...
    compileSdk = 36
    namespace = "com.flowspeed.link.android"
    buildTypes {
        debug {
            applicationIdSuffix = ".debug"
            resValue("string", "app_short_name", "Flow - Debug")
        }
    }
    buildFeatures {
        compose = true
...
androidComponents.onVariants { variant ->
    tasks.register(
        ...
        keystoreUri.set(provider {
            getFromEnvOrProperties("FLOW_KEYSTORE_FILE")
        })
        keystorePassword.set(provider {
            getFromEnvOrProperties("FLOW_KEYSTORE_FILE_PASSWORD")
        })
        keyPassword.set(provider {
            getFromEnvOrProperties("FLOW_KEYSTORE_KEY_PASSWORD")
        })
        keyAlias.set(provider {
            getFromEnvOrProperties("FLOW_KEYSTORE_KEY_ALIAS")
        })
    }
}
